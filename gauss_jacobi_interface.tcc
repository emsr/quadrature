...
template<typename _Tp>
  ...
  jac_quadrature<_Tp>::quadrature_zwd()
  {
	...
	err = this->zeros_gj();
	if (err)
	  __gnu_cxx::__throw__IntegrationError("Problem calculating the zeros", err, _Tp{}, _Tp{});
	err = this->weights_gj();
	if (err)
	  __gnu_cxx::__throw__IntegrationError("Problem calculating the weightd", err, _Tp{}, _Tp{});
	err = this->diffmat_gj();
	if (err)
	  __gnu_cxx::__throw__IntegrationError("Problem calculating the differentiation matrix", err, _Tp{}, _Tp{});
	break;
      case Gauss_Lobatto:
	err = this->zeros_glj();
	if (err)
	...
	break;
      default:
	__gnu_cxx::__throw__IntegrationError("Illegal quadrature type", err, _Tp{}, _Tp{});
      }
