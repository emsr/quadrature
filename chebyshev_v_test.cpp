	...
	for (int n2 = 0; n2 <= n1; ++n2)
	  {
	    std::function<_Tp(_Tp)>
	      func([n1, n2](_Tp x)->_Tp{return normalized_chebyshev_v(n1, n2, x);});
...
//		= integrate(func, _Tp{-1}, _Tp{1}, integ_precision, _Tp{0});
	...
	std::cout << "Integration successful for chebyshev_v polynomials up to n = " << n1
		  << '\n' << std::flush;
      }
	...
	for (int n2 = 0; n2 <= itop; n2 += del)
	  {
	    std::function<_Tp(_Tp)>
	      func([itop, n2](_Tp x)->_Tp{return normalized_chebyshev_v(itop, n2, x);});
	    const _Tp integ_precision = _Tp{1000} * eps;
	    const _Tp comp_precision = _Tp{10} * integ_precision;
